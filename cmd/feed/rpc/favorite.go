...
func MCheckFavorite(ctx context.Context, req *favorite.MCheckFavoriteRequest) (*favorite.MCheckFavoriteResponse, error) {
	resp, err := favoriteClient.MCheckFavorite(ctx, req)
	if err != nil {
		return nil, err
	}
	if _, ok := erren.ErrorMap[resp.StatusCode]; ok {
...
func MCountFavorite(ctx context.Context, req *favorite.MCountVideoFavoriteRequest) (*favorite.MCountVideoFavoriteResponse, error) {
