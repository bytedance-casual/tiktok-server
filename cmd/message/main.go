...
func Init() {
	conf.Init()
	tracer.InitJaeger(conf.MessageServiceName)
	dal.Init()
}
...
func main() {
	...
	svr := message.NewServer(new(MessageServiceImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: conf.MessageServiceName}), // server name
